...
namespace jelly
{
	...
	namespace Impl
	{
		...
		Host::ReadWALStream(
			...
			uint32_t					aId) 
		{
			std::unique_ptr<Impl::FileStreamReader> f(new Impl::FileStreamReader(
		...
		Host::CreateWAL(
			...
			uint32_t					aId) 
		{
			std::unique_ptr<Impl::WALWriter> f(new Impl::WALWriter(
				PathUtils::MakePath(m_root.c_str(), PathUtils::FILE_TYPE_WAL, aNodeId, aId).c_str(),
		...
		Host::CreateStore(
			...
			uint32_t					aId) 
		{
			std::unique_ptr<Impl::StoreWriter> f(new Impl::StoreWriter(
				PathUtils::MakePath(m_root.c_str(), PathUtils::FILE_TYPE_STORE, aNodeId, aId).c_str()));
