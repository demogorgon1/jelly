		...
		namespace FileTest
		{
			...
			Run()
			{
				...
				// Create a file
				{
					...
					JELLY_ALWAYS_ASSERT(fileStats.m_counters[FileStats::ID_WRITE] == testDataSize);
				}
				...
				// Read file as a stream
				{
					...
					char buffer[256];
					JELLY_UNUSED(buffer);
					JELLY_ALWAYS_ASSERT(sizeof(buffer) >= testDataSize);
					JELLY_ALWAYS_ASSERT(f.Read(buffer, testDataSize) == testDataSize);
					JELLY_ALWAYS_ASSERT(memcmp(buffer, TEST_DATA, testDataSize) == 0);
					JELLY_ALWAYS_ASSERT(fileStats.m_counters[FileStats::ID_READ] == testDataSize);
					JELLY_ALWAYS_ASSERT(fileStats.m_counters[FileStats::ID_WRITE] == testDataSize);
				}
				...
				// Read file with random access
				{
					...
					JELLY_ALWAYS_ASSERT(strcmp(buffer, "HelloWorld") == 0);
					JELLY_ALWAYS_ASSERT(fileStats.m_counters[FileStats::ID_READ] == testDataSize + 10);
					JELLY_ALWAYS_ASSERT(fileStats.m_counters[FileStats::ID_WRITE] == testDataSize);
				}
				...
				// Open a file as a stream and while still open, also open it as random access
				{
					...
					JELLY_ALWAYS_ASSERT(fStream.IsValid());

					{
						File fRandom(&context, "testfile.tmp", File::MODE_READ_RANDOM, FileHeader());
						JELLY_ALWAYS_ASSERT(fRandom.IsValid());
					}
				}
				...
				JELLY_ALWAYS_ASSERT(std::filesystem::remove("testfile.tmp"));
			}
