...
namespace jelly
{
	...
	enum RequestResult
	{
		...
		REQUEST_RESULT_ERROR,				//!< Error occured while processing request (call Request::GetError() for details)
		REQUEST_RESULT_ALREADY_LOCKED,		//!< LockNode: the key was already locked with a different lock id
		REQUEST_RESULT_NOT_LOCKED,			//!< LockNode: the key wasn't locked by the request lock id 
